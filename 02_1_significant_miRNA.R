...
# --- GroupInfo -----------------------------------------------------------------------------------
diseaseStage.ALL <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="sample information", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
...
miRNA.rawData <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(raw)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
miRNA.normalizedData <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(normalized)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
miRNA.normalizedData.HC <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(normalized)(HC)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
miRNA.normalizedData.PDND <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(normalized)(PDND)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
miRNA.normalizedData.PDMCI <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(normalized)(PD-MCI)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
miRNA.normalizedData.PDD <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(normalized)(PDD)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
miRNA.normalizedData.PD <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(normalized)(PD)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
miRNA.normalizedData.MSA <- read.xlsx("./Data/01_datatable/miRNA_datatable.xlsx", sheet="miRNA(normalized)(MSA)", colNames=TRUE, rowNames=FALSE, check.names=FALSE)
...
for(m in 1:ncol(combn.idx))
{
	...
	mainDir <- './Data/02_significant/miRNA'
	...
	miRNA.normalizedData.Group1 <- eval(parse(text=paste('miRNA.normalizedData.', Group[1], sep='')))
	miRNA.normalizedData.Group2 <- eval(parse(text=paste('miRNA.normalizedData.', Group[2], sep='')))
	
	# miRNA.normalizedData.HC_PDND, miRNA.normalizedData.HC_PDMCI, miRNA.normalizedData.HC_PDD, miRNA.normalizedData.HC_PD, miRNA.normalizedData.HC_MSA
	# miRNA.normalizedData.PDND_PDMCI, miRNA.normalizedData.PDND_PDD, miRNA.normalizedData.PDND_PD, miRNA.normalizedData.PDND_MSA
	# miRNA.normalizedData.PDMCI_PDD, miRNA.normalizedData.PDMCI_PD, miRNA.normalizedData.PDMCI_MSA
	# miRNA.normalizedData.PDD_PD, miRNA.normalizedData.PDD_MSA
	# miRNA.normalizedData.PD_MSA
	assign(paste('miRNA.normalizedData.', combnName, sep=''), merge(miRNA.normalizedData.Group1, miRNA.normalizedData.Group2, sort=FALSE))
	miRNA.normalizedData.Comp <- eval(parse(text=paste('miRNA.normalizedData.', combnName, sep='')))
	rownames(miRNA.normalizedData.Comp) = unlist(sapply(strsplit(miRNA.normalizedData.Comp[,'Name'], '/'), FUN=function(X) X[1]))
	
	# col.idx
	miRNA.annotation = c('type','Name','Accession_ID','Drives_from','locus_GRCh38','locus_hg19')
	miRNA.annotation.idx = c(
		which(colnames(miRNA.normalizedData.Comp)==miRNA.annotation[1]), 
		which(colnames(miRNA.normalizedData.Comp)==miRNA.annotation[2]), 
		which(colnames(miRNA.normalizedData.Comp)==miRNA.annotation[3]), 
		which(colnames(miRNA.normalizedData.Comp)==miRNA.annotation[4]), 
		which(colnames(miRNA.normalizedData.Comp)==miRNA.annotation[5]), 
		which(colnames(miRNA.normalizedData.Comp)==miRNA.annotation[6]))
	
	samples_serial_num = sapply(strsplit(colnames(miRNA.normalizedData.Comp)[(length(miRNA.annotation)+1):ncol(miRNA.normalizedData.Comp)], '_'), FUN=function(X) X[1])
	samples_id = sapply(strsplit(colnames(miRNA.normalizedData.Comp)[(length(miRNA.annotation)+1):ncol(miRNA.normalizedData.Comp)], '_'), FUN=function(X) X[2])
	
	Group1.colidx = unlist(sapply(diseaseStage.Group1[,'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
	Group2.colidx = unlist(sapply(diseaseStage.Group2[,'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
	
	if(Group[1] == 'PD' | Group[2] == 'PD') {
		if(Group[1] == 'PD') {
			PDND.colidx = unlist(sapply(diseaseStage.Group1[which(diseaseStage.Group1[,'Symp Group']=='PDND'),'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
			PDMCI.colidx = unlist(sapply(diseaseStage.Group1[which(diseaseStage.Group1[,'Symp Group']=='PD-MCI'),'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
			PDD.colidx = unlist(sapply(diseaseStage.Group1[which(diseaseStage.Group1[,'Symp Group']=='PDD'),'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
		}
		if(Group[2] == 'PD') {
			PDND.colidx = unlist(sapply(diseaseStage.Group2[which(diseaseStage.Group2[,'Symp Group']=='PDND'),'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
			PDMCI.colidx = unlist(sapply(diseaseStage.Group2[which(diseaseStage.Group2[,'Symp Group']=='PD-MCI'),'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
			PDD.colidx = unlist(sapply(diseaseStage.Group2[which(diseaseStage.Group2[,'Symp Group']=='PDD'),'Sample ID'], FUN=function(X) which(samples_id %in% X) + length(miRNA.annotation)))
		}
	} else {
		PDND.colidx = NULL
		PDMCI.colidx = NULL
	...
	mainDir <- paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],sep='')
	subDir <- 'Boxplot'
	if(file.exists(file.path(mainDir, subDir))) {
	} else {
		dir.create(file.path(mainDir, subDir))
	...
	# subset
	Group1.miRNA.rowidx <- c()
	Group2.miRNA.rowidx <- c()
	Group1_Group2.miRNA.rowidx <- c()
	NA.miRNA.rowidx <- c()
	common_miRNA.ttest.pvalue <- c()
	#common_miRNA.ANOVA.pvalue <- c()
	common_miRNA.log2_FC <- c()
	common_miRNA.FC <- c()
	for(i in 1:nrow(miRNA.normalizedData.Comp))
	{
		Group1.value = as.numeric(miRNA.normalizedData.Comp[i, Group1.colidx])
		Group2.value = as.numeric(miRNA.normalizedData.Comp[i, Group2.colidx])
		...
		main_title = unlist(strsplit(miRNA.normalizedData.Comp[i,'Name'], '/'))[1]
		
		sampleValue = miRNA.normalizedData.Comp[i, , drop=FALSE]
		sampleGroup = c(rep(NA, length(miRNA.annotation)), rep(Group[1], length(Group1.colidx)), rep(Group[2], length(Group2.colidx)))
		
		sampleValueGroup = as.data.frame(t(rbind(sampleValue, sampleGroup)[, -miRNA.annotation.idx, drop=FALSE]), stringsAsFactors=FALSE)
		...
		if(sum(Group1.value, na.rm=TRUE)!=0 & sum(Group2.value, na.rm=TRUE)==0){
			Group1.miRNA.rowidx <- c(Group1.miRNA.rowidx, i)
			
			mainDir <- paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot',sep='')
			subDir <- Group[1]
			if(file.exists(file.path(mainDir, subDir))) {
			} else {
				dir.create(file.path(mainDir, subDir))
			...
			# /// boxplot {graphics} ///
			#png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],'/',gsub('/', '.', miRNA.normalizedData.Comp[i,'Name']),'.png',sep=''), width=4, height=4, units="in", res=600)
			#boxplot(Value~Group, data=sampleValueGroup, main=main_title, xlab="Symp Group", ylab="miRNA(normalized)")
			#dev.off()
			...
			Group2.miRNA.rowidx <- c(Group2.miRNA.rowidx, i)
			...
			# /// boxplot {graphics} ///
			#png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[2],'/',gsub('/', '.', miRNA.normalizedData.Comp[i,'Name']),'.png',sep=''), width=4, height=4, units="in", res=600)
			#boxplot(Value~Group, data=sampleValueGroup, main=main_title, xlab="Symp Group", ylab="miRNA(normalized)")
			#dev.off()
			...
			Group1_Group2.miRNA.rowidx <- c(Group1_Group2.miRNA.rowidx, i)
			
			# t-test
			miRNA.ttest = t.test(x=Group1.value, y=Group2.value, alternative="two.sided")
			miRNA.ttest.pvalue = miRNA.ttest$p.value
			miRNA.ttest$method		# "Welch Two Sample t-test"
			common_miRNA.ttest.pvalue <- c(common_miRNA.ttest.pvalue, miRNA.ttest.pvalue)
			
			# one-way ANOVA test
			#miRNA.ANOVA = aov(formula=Value~Group, data=sampleValueGroup)
			#miRNA.ANOVA.Summary = summary(miRNA.ANOVA)
			#miRNA.ANOVA.pvalue = miRNA.ANOVA.Summary[[1]]$'Pr(>F)'[1]
			#common_miRNA.ANOVA.pvalue <- c(common_miRNA.ANOVA.pvalue, miRNA.ANOVA.pvalue)
			...
			common_miRNA.log2_FC <- c(common_miRNA.log2_FC, log2_FC)
			common_miRNA.FC <- c(common_miRNA.FC, FC)
		} else if(sum(Group1.value, na.rm=TRUE)==0 & sum(Group2.value, na.rm=TRUE)==0){
			NA.miRNA.rowidx <- c(NA.miRNA.rowidx, i)
		}
		...
		if(ggplot.plotidx == 1) {
			...
			pic <- pic + 
				labs(title = main_title, x = "Symp Group", y = "miRNA(normalized)", fill = "Symp Group", color = "Symp Group") + 
	...
	# all.ZeroCount
	Group1.all.ZeroCount = table(sapply(c(1:nrow(miRNA.normalizedData.Comp)), FUN=function(X) length(which(miRNA.normalizedData.Comp[X, Group1.colidx] == 0 | is.na(miRNA.normalizedData.Comp[X, Group1.colidx])))))
	Group2.all.ZeroCount = table(sapply(c(1:nrow(miRNA.normalizedData.Comp)), FUN=function(X) length(which(miRNA.normalizedData.Comp[X, Group2.colidx] == 0 | is.na(miRNA.normalizedData.Comp[X, Group2.colidx])))))
	...
	if(Group[1] == 'PD' | Group[2] == 'PD') {
		PDND.all.ZeroCount = table(sapply(c(1:nrow(miRNA.normalizedData.Comp)), FUN=function(X) length(which(miRNA.normalizedData.Comp[X, PDND.colidx] == 0 | is.na(miRNA.normalizedData.Comp[X, PDND.colidx])))))
		PDMCI.all.ZeroCount = table(sapply(c(1:nrow(miRNA.normalizedData.Comp)), FUN=function(X) length(which(miRNA.normalizedData.Comp[X, PDMCI.colidx] == 0 | is.na(miRNA.normalizedData.Comp[X, PDMCI.colidx])))))
		PDD.all.ZeroCount = table(sapply(c(1:nrow(miRNA.normalizedData.Comp)), FUN=function(X) length(which(miRNA.normalizedData.Comp[X, PDD.colidx] == 0 | is.na(miRNA.normalizedData.Comp[X, PDD.colidx])))))
		if(Group[1] == 'PD') {
	...
	# common.ZeroCount
	Group1.common.ZeroCount = table(sapply(c(1:length(Group1_Group2.miRNA.rowidx)), FUN=function(X) length(which(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], Group1.colidx] == 0 | is.na(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], Group1.colidx])))))
	Group2.common.ZeroCount = table(sapply(c(1:length(Group1_Group2.miRNA.rowidx)), FUN=function(X) length(which(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], Group2.colidx] == 0 | is.na(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], Group2.colidx])))))
	...
	if(Group[1] == 'PD' | Group[2] == 'PD') {
		PDND.common.ZeroCount = table(sapply(c(1:length(Group1_Group2.miRNA.rowidx)), FUN=function(X) length(which(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], PDND.colidx] == 0 | is.na(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], PDND.colidx])))))
		PDMCI.common.ZeroCount = table(sapply(c(1:length(Group1_Group2.miRNA.rowidx)), FUN=function(X) length(which(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], PDMCI.colidx] == 0 | is.na(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], PDMCI.colidx])))))
		PDD.common.ZeroCount = table(sapply(c(1:length(Group1_Group2.miRNA.rowidx)), FUN=function(X) length(which(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], PDD.colidx] == 0 | is.na(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx[X], PDD.colidx])))))
		if(Group[1] == 'PD') {
	...
	for(z in 1:2)
	{
		if(z == 1) {
			...
			png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','miRNA_ZeroValueDistribution ','(',Group[1],' vs ',Group[2],' (All)',')','.png',sep=''), width=12, height=6, units="in", res=600)
		} else if(z == 2) {
			...
			png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','miRNA_ZeroValueDistribution ','(',Group[1],' vs ',Group[2],' (Common)',')','.png',sep=''), width=12, height=6, units="in", res=600)
		}
		...
		pic <- pic + theme_bw() #+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
		pic <- pic + 
			labs(title = "Zero value distribution", x = "Number of sample with zero value", y = "Number of miRNA") + 
	...
	common_miRNA.ttest.pvalue_adjust = p.adjust(common_miRNA.ttest.pvalue, method="fdr")
	#common_miRNA.ANOVA.pvalue_adjust = p.adjust(common_miRNA.ANOVA.pvalue, method="fdr")
	
	miRNA.normalizedData.Comp.stat = cbind(miRNA.normalizedData.Comp[Group1_Group2.miRNA.rowidx, , drop=FALSE], common_miRNA.ttest.pvalue, common_miRNA.ttest.pvalue_adjust, common_miRNA.log2_FC, common_miRNA.FC, stringsAsFactors=FALSE)
	colnames(miRNA.normalizedData.Comp.stat)[(ncol(miRNA.normalizedData.Comp.stat)-3):ncol(miRNA.normalizedData.Comp.stat)] = c('pvalue','pvalue_FDR','log2_FC','FC')
	miRNA.normalizedData.Comp.stat = miRNA.normalizedData.Comp.stat[, c(miRNA.annotation.idx, (ncol(miRNA.normalizedData.Comp.stat)-3):ncol(miRNA.normalizedData.Comp.stat), (length(miRNA.annotation.idx)+1):(ncol(miRNA.normalizedData.Comp.stat)-4)), drop=FALSE]
	
	# significant index
	significant_miRNA.idx = Group1_Group2.miRNA.rowidx[which(miRNA.normalizedData.Comp.stat[,'pvalue'] < pvalue.cutoff)]
	nonsignificant_miRNA.idx = Group1_Group2.miRNA.rowidx[which(miRNA.normalizedData.Comp.stat[,'pvalue'] >= pvalue.cutoff)]
	
	significant_miRNA_FCup.idx = Group1_Group2.miRNA.rowidx[which(miRNA.normalizedData.Comp.stat[,'pvalue'] < pvalue.cutoff & miRNA.normalizedData.Comp.stat[,'FC'] > 0)]
	significant_miRNA_FCdown.idx = Group1_Group2.miRNA.rowidx[which(miRNA.normalizedData.Comp.stat[,'pvalue'] < pvalue.cutoff & miRNA.normalizedData.Comp.stat[,'FC'] < 0)]
	significant_miRNA_noFC.idx = Group1_Group2.miRNA.rowidx[which(miRNA.normalizedData.Comp.stat[,'pvalue'] < pvalue.cutoff & miRNA.normalizedData.Comp.stat[,'FC'] == 0)]
	
	if(length(significant_miRNA.idx) > 0) {
		mainDir <- paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot',sep='')
		subDir <- paste(Group[1],' vs ',Group[2],sep='')
		if(file.exists(file.path(mainDir, subDir))) {
		} else {
		...
		mainDir <- paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],sep='')
		subDir <- 'significant'
		if(file.exists(file.path(mainDir, subDir))) {
		} else {
			dir.create(file.path(mainDir, subDir))
		...
		for(i in 1:length(significant_miRNA.idx))
		{
			main_title = unlist(strsplit(miRNA.normalizedData.Comp[significant_miRNA.idx[i],'Name'], '/'))[1]
			
			sampleValue = miRNA.normalizedData.Comp[significant_miRNA.idx[i], , drop=FALSE]
			sampleGroup = c(rep(NA, length(miRNA.annotation)), rep(Group[1], length(Group1.colidx)), rep(Group[2], length(Group2.colidx)))
			...
			ggplot.plotidx = 0
			if(significant_miRNA.idx[i] %in% significant_miRNA_FCup.idx) {
				mainDir <- paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant',sep='')
				subDir <- 'FCup'
				if(file.exists(file.path(mainDir, subDir))) {
				} else {
					dir.create(file.path(mainDir, subDir))
				...
				# /// boxplot {graphics} ///
				#png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant/FCup/',gsub('/', '.', miRNA.normalizedData.Comp[significant_miRNA.idx[i],'Name']),'.png',sep=''), width=4, height=4, units="in", res=600)
				#boxplot(Value~Group, data=sampleValueGroup, main=main_title, xlab="Symp Group", ylab="miRNA(normalized)")
				#dev.off()
				...
				png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant/FCup/',gsub('/', '.', miRNA.normalizedData.Comp[significant_miRNA.idx[i],'Name']),'.png',sep=''), width=4, height=4, units="in", res=600)
				ggplot.plotidx = 1
			} else if(significant_miRNA.idx[i] %in% significant_miRNA_FCdown.idx) {
				mainDir <- paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant',sep='')
				subDir <- 'FCdown'
				if(file.exists(file.path(mainDir, subDir))) {
				} else {
				...
				# /// boxplot {graphics} ///
				#png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant/FCdown/',gsub('/', '.', miRNA.normalizedData.Comp[significant_miRNA.idx[i],'Name']),'.png',sep=''), width=4, height=4, units="in", res=600)
				#boxplot(Value~Group, data=sampleValueGroup, main=main_title, xlab="Symp Group", ylab="miRNA(normalized)")
				#dev.off()
				...
				png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant/FCdown/',gsub('/', '.', miRNA.normalizedData.Comp[significant_miRNA.idx[i],'Name']),'.png',sep=''), width=4, height=4, units="in", res=600)
				ggplot.plotidx = 1
			} else if(significant_miRNA.idx[i] %in% significant_miRNA_noFC.idx) {
				mainDir <- paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant',sep='')
				subDir <- 'noFC'
				if(file.exists(file.path(mainDir, subDir))) {
				} else {
				...
				# /// boxplot {graphics} ///
				#png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Boxplot','/',Group[1],' vs ',Group[2],'/significant/noFC/',gsub('/', '.', miRNA.normalizedData.Comp[significant_miRNA.idx[i],'Name']),'.png',sep=''), width=4, height=4, units="in", res=600)
				#boxplot(Value~Group, data=sampleValueGroup, main=main_title, xlab="Symp Group", ylab="miRNA(normalized)")
				#dev.off()
	...
	sampleStat = miRNA.normalizedData.Comp.stat[, c('Name','pvalue','pvalue_FDR','log2_FC','FC'), drop=FALSE]
	...
	png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Volcano','/','miRNA_volcano ','(',Group[1],' vs ',Group[2],')','.png',sep=''), width=6, height=6, units="in", res=600)
	...
	# combine specific miRNA and significant common miRNA
	# Group1
	miRNA.normalizedData.Comp.Group1 <- c()
	if(length(Group1.miRNA.rowidx) > 0) {
		# specific miRNA ordered by "non-zero sample number count" and "mean of non-zero sample value"
		Group1.nonZeroCount = sapply(c(1:length(Group1.miRNA.rowidx)), FUN=function(X) length(which(miRNA.normalizedData.Comp[Group1.miRNA.rowidx[X], Group1.colidx] != 0)))
		Group1.nonZeroMean = sapply(c(1:length(Group1.miRNA.rowidx)), FUN=function(X) mean(as.numeric(miRNA.normalizedData.Comp[Group1.miRNA.rowidx[X], Group1.colidx][which(miRNA.normalizedData.Comp[Group1.miRNA.rowidx[X], Group1.colidx] != 0)]), na.rm=TRUE))
		Group1.miRNA.rowidx = Group1.miRNA.rowidx[order(Group1.nonZeroCount, Group1.nonZeroMean, decreasing=TRUE)]
		
		# specific miRNA
		miRNA.normalizedData.Comp.Group1 = cbind(miRNA.normalizedData.Comp[Group1.miRNA.rowidx, , drop=FALSE], pvalue=rep(NA, length(Group1.miRNA.rowidx)), pvalue_FDR=rep(NA, length(Group1.miRNA.rowidx)), log2_FC=rep(NA, length(Group1.miRNA.rowidx)), FC=rep(NA, length(Group1.miRNA.rowidx)), stringsAsFactors=FALSE)
		miRNA.normalizedData.Comp.Group1 = miRNA.normalizedData.Comp.Group1[, c(miRNA.annotation.idx, (ncol(miRNA.normalizedData.Comp.Group1)-3):ncol(miRNA.normalizedData.Comp.Group1), (length(miRNA.annotation.idx)+1):(ncol(miRNA.normalizedData.Comp.Group1)-4)), drop=FALSE]
	}
	# Group2
	miRNA.normalizedData.Comp.Group2 <- c()
	if(length(Group2.miRNA.rowidx) > 0) {
		# specific miRNA ordered by "non-zero sample number count" and "mean of non-zero sample value"
		Group2.nonZeroCount = sapply(c(1:length(Group2.miRNA.rowidx)), FUN=function(X) length(which(miRNA.normalizedData.Comp[Group2.miRNA.rowidx[X], Group2.colidx] != 0)))
		Group2.nonZeroMean = sapply(c(1:length(Group2.miRNA.rowidx)), FUN=function(X) mean(as.numeric(miRNA.normalizedData.Comp[Group2.miRNA.rowidx[X], Group2.colidx][which(miRNA.normalizedData.Comp[Group2.miRNA.rowidx[X], Group2.colidx] != 0)]), na.rm=TRUE))
		Group2.miRNA.rowidx = Group2.miRNA.rowidx[order(Group2.nonZeroCount, Group2.nonZeroMean, decreasing=TRUE)]
		
		# specific miRNA
		miRNA.normalizedData.Comp.Group2 = cbind(miRNA.normalizedData.Comp[Group2.miRNA.rowidx, , drop=FALSE], pvalue=rep(NA, length(Group2.miRNA.rowidx)), pvalue_FDR=rep(NA, length(Group2.miRNA.rowidx)), log2_FC=rep(NA, length(Group2.miRNA.rowidx)), FC=rep(NA, length(Group2.miRNA.rowidx)), stringsAsFactors=FALSE)
		miRNA.normalizedData.Comp.Group2 = miRNA.normalizedData.Comp.Group2[, c(miRNA.annotation.idx, (ncol(miRNA.normalizedData.Comp.Group2)-3):ncol(miRNA.normalizedData.Comp.Group2), (length(miRNA.annotation.idx)+1):(ncol(miRNA.normalizedData.Comp.Group2)-4)), drop=FALSE]
	}
	
	# significant common miRNA
	miRNA.normalizedData.Comp.stat.FCup = miRNA.normalizedData.Comp.stat[which(miRNA.normalizedData.Comp.stat[,'pvalue'] < pvalue.cutoff & miRNA.normalizedData.Comp.stat[,'FC'] > 0), , drop=FALSE]
	miRNA.normalizedData.Comp.stat.FCup = miRNA.normalizedData.Comp.stat.FCup[order(miRNA.normalizedData.Comp.stat.FCup[,'pvalue']), , drop=FALSE]
	miRNA.normalizedData.Comp.stat.FCdown = miRNA.normalizedData.Comp.stat[which(miRNA.normalizedData.Comp.stat[,'pvalue'] < pvalue.cutoff & miRNA.normalizedData.Comp.stat[,'FC'] < 0), , drop=FALSE]
	miRNA.normalizedData.Comp.stat.FCdown = miRNA.normalizedData.Comp.stat.FCdown[order(miRNA.normalizedData.Comp.stat.FCdown[,'pvalue']), , drop=FALSE]
	
	# Group1 > Group2
	if(!is.null(miRNA.normalizedData.Comp.Group1)) {
		df1 = miRNA.normalizedData.Comp.Group1
		df2 = miRNA.normalizedData.Comp.stat.FCdown
		df3 = rbind(
		...
		miRNA.normalizedData.Comp.UPinGroup1 = df3
	} else {
		miRNA.normalizedData.Comp.UPinGroup1 = miRNA.normalizedData.Comp.stat.FCdown
	}
	# Group1 < Group2
	if(!is.null(miRNA.normalizedData.Comp.Group2)) {
		df1 = miRNA.normalizedData.Comp.Group2
		df2 = miRNA.normalizedData.Comp.stat.FCup
		df3 = rbind(
			data.frame(c(df1, sapply(setdiff(names(df2), names(df1)), function(x) NA)), check.names = FALSE, stringsAsFactors = FALSE), 
			data.frame(c(df2, sapply(setdiff(names(df1), names(df2)), function(x) NA)), check.names = FALSE, stringsAsFactors = FALSE)
		)
		...
		miRNA.normalizedData.Comp.UPinGroup2 = df3
	} else {
		miRNA.normalizedData.Comp.UPinGroup2 = miRNA.normalizedData.Comp.stat.FCup
	}
	...
	for(n in 1:6)
	{
		...
		if(n == 1) {
			mat1 = as.matrix(miRNA.normalizedData.Comp.UPinGroup1[, Group1.colidx.new, drop=FALSE])
			mat2 = as.matrix(miRNA.normalizedData.Comp.UPinGroup1[, setdiff(Group2.colidx.new,Group1.colidx.new), drop=FALSE])
			mat3 = as.matrix(miRNA.normalizedData.Comp.UPinGroup1[, unique(c(Group1.colidx.new,Group2.colidx.new)), drop=FALSE])
			
			heatmap.sep.plot <- paste('miRNA_heatmap_by_','High',Group[1],' ','(',Group[1],' + ',Group[2],' (Specific+Common)',')',sep='')
			heatmap.mix.plot <- paste('miRNA_heatmap_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			dendrogram.plot <- paste('miRNA_dendrogram_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			pca2D.plot <- paste('miRNA_pca2D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			pca3D.plot <- paste('miRNA_pca3D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			plsda2D.plot <- paste('miRNA_plsda2D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			plsda3D.plot <- paste('miRNA_plsda3D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
		} else if(n == 2) {
			mat1 = as.matrix(miRNA.normalizedData.Comp.UPinGroup2[, Group1.colidx.new, drop=FALSE])
			mat2 = as.matrix(miRNA.normalizedData.Comp.UPinGroup2[, setdiff(Group2.colidx.new,Group1.colidx.new), drop=FALSE])
			mat3 = as.matrix(miRNA.normalizedData.Comp.UPinGroup2[, unique(c(Group1.colidx.new,Group2.colidx.new)), drop=FALSE])
			
			heatmap.sep.plot <- paste('miRNA_heatmap_by_','High',Group[2],' ','(',Group[1],' + ',Group[2],' (Specific+Common)',')',sep='')
			heatmap.mix.plot <- paste('miRNA_heatmap_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			dendrogram.plot <- paste('miRNA_dendrogram_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			pca2D.plot <- paste('miRNA_pca2D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			pca3D.plot <- paste('miRNA_pca3D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			plsda2D.plot <- paste('miRNA_plsda2D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			plsda3D.plot <- paste('miRNA_plsda3D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
		} else if(n == 3) {
			mat1 = as.matrix(rbind(miRNA.normalizedData.Comp.UPinGroup1, miRNA.normalizedData.Comp.UPinGroup2)[, Group1.colidx.new, drop=FALSE])
			mat2 = as.matrix(rbind(miRNA.normalizedData.Comp.UPinGroup1, miRNA.normalizedData.Comp.UPinGroup2)[, setdiff(Group2.colidx.new,Group1.colidx.new), drop=FALSE])
			mat3 = as.matrix(rbind(miRNA.normalizedData.Comp.UPinGroup1, miRNA.normalizedData.Comp.UPinGroup2)[, unique(c(Group1.colidx.new,Group2.colidx.new)), drop=FALSE])
			
			heatmap.sep.plot <- paste('miRNA_heatmap_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' + ',Group[2],' (Specific+Common)',')',sep='')
			heatmap.mix.plot <- paste('miRNA_heatmap_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			dendrogram.plot <- paste('miRNA_dendrogram_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			pca2D.plot <- paste('miRNA_pca2D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			pca3D.plot <- paste('miRNA_pca3D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			plsda2D.plot <- paste('miRNA_plsda2D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
			plsda3D.plot <- paste('miRNA_plsda3D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Specific+Common)',')',sep='')
		} else if(n == 4) {
			mat1 = as.matrix(miRNA.normalizedData.Comp.stat.FCdown[, Group1.colidx.new, drop=FALSE])
			mat2 = as.matrix(miRNA.normalizedData.Comp.stat.FCdown[, setdiff(Group2.colidx.new,Group1.colidx.new), drop=FALSE])
			mat3 = as.matrix(miRNA.normalizedData.Comp.stat.FCdown[, unique(c(Group1.colidx.new,Group2.colidx.new)), drop=FALSE])
			
			heatmap.sep.plot <- paste('miRNA_heatmap_by_','High',Group[1],' ','(',Group[1],' + ',Group[2],' (Common)',')',sep='')
			heatmap.mix.plot <- paste('miRNA_heatmap_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			dendrogram.plot <- paste('miRNA_dendrogram_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			pca2D.plot <- paste('miRNA_pca2D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			pca3D.plot <- paste('miRNA_pca3D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			plsda2D.plot <- paste('miRNA_plsda2D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			plsda3D.plot <- paste('miRNA_plsda3D_by_','High',Group[1],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
		} else if(n == 5) {
			mat1 = as.matrix(miRNA.normalizedData.Comp.stat.FCup[, Group1.colidx.new, drop=FALSE])
			mat2 = as.matrix(miRNA.normalizedData.Comp.stat.FCup[, setdiff(Group2.colidx.new,Group1.colidx.new), drop=FALSE])
			mat3 = as.matrix(miRNA.normalizedData.Comp.stat.FCup[, unique(c(Group1.colidx.new,Group2.colidx.new)), drop=FALSE])
			
			heatmap.sep.plot <- paste('miRNA_heatmap_by_','High',Group[2],' ','(',Group[1],' + ',Group[2],' (Common)',')',sep='')
			heatmap.mix.plot <- paste('miRNA_heatmap_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			dendrogram.plot <- paste('miRNA_dendrogram_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			pca2D.plot <- paste('miRNA_pca2D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			pca3D.plot <- paste('miRNA_pca3D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			plsda2D.plot <- paste('miRNA_plsda2D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			plsda3D.plot <- paste('miRNA_plsda3D_by_','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
		} else if(n == 6) {
			mat1 = as.matrix(rbind(miRNA.normalizedData.Comp.stat.FCdown, miRNA.normalizedData.Comp.stat.FCup)[, Group1.colidx.new, drop=FALSE])
			mat2 = as.matrix(rbind(miRNA.normalizedData.Comp.stat.FCdown, miRNA.normalizedData.Comp.stat.FCup)[, setdiff(Group2.colidx.new,Group1.colidx.new), drop=FALSE])
			mat3 = as.matrix(rbind(miRNA.normalizedData.Comp.stat.FCdown, miRNA.normalizedData.Comp.stat.FCup)[, unique(c(Group1.colidx.new,Group2.colidx.new)), drop=FALSE])
			
			heatmap.sep.plot <- paste('miRNA_heatmap_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' + ',Group[2],' (Common)',')',sep='')
			heatmap.mix.plot <- paste('miRNA_heatmap_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			dendrogram.plot <- paste('miRNA_dendrogram_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			pca2D.plot <- paste('miRNA_pca2D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			pca3D.plot <- paste('miRNA_pca3D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			plsda2D.plot <- paste('miRNA_plsda2D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
			plsda3D.plot <- paste('miRNA_plsda3D_by_','High',Group[1],'+','High',Group[2],' ','(',Group[1],' vs ',Group[2],' (Common)',')',sep='')
		}
		...
		if(nrow(mat1) > 0) {
			...
			png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Heatmap','/',heatmap.sep.plot,'.png',sep=''), width=width.inch, height=height.inch, units="in", res=600)
			ht_list = ht1 + ht2
		...
		if(nrow(mat3) > 0) {
			...
			png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Heatmap','/',heatmap.mix.plot,'.png',sep=''), width=width.inch, height=height.inch, units="in", res=600)
			ht3 = Heatmap(mat3, 
						  name = "normalized", 
		...
		if(nrow(mat) > 0) {
			...
			png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','Dendrogram','/',dendrogram.plot,'.png',sep=''), width=width.inch, height=6, units="in", res=600)
			...
			if(ncol(df) >= 2) {
				...
				#png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','PCA','/',pca2D.plot,'.png',sep=''), width=10, height=10, units="in", res=600)
				...
				#ggsave(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','PCA','/',pca2D.plot,'.png',sep=''), pic, width=10, height=10, units="in", dpi=600, bg="transparent")
				...
				if(ncol(df) >= 3) {
					...
					snapshot3d(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','PCA','/',pca3D.plot,'.png',sep=''))
					rgl.close()
				}
			}
			...
			if(ncol(df) >= 3) {
				...
				png(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','PLSDA','/',plsda2D.plot,'.png',sep=''), width=10, height=10, units="in", res=600)
				...
				snapshot3d(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','PLSDA','/',plsda3D.plot,'.png',sep=''))
				rgl.close()
	...
	wb <- openxlsx::createWorkbook(paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','miRNA_datatable ','(',Group[1],' vs ',Group[2],')','.xlsx',sep=''))
	...
	for(s in 1:10)
	{
		if(s == 1) {
			...
			sheet = paste('miRNA(normalized)','(',Group[1],',',Group[2],')',sep='')
			sheetData = miRNA.normalizedData.Comp
		} else if(s == 3) {
			sheet = paste('Specific miRNA in ',Group[1],sep='')
			sheetData = miRNA.normalizedData.Comp[Group1.miRNA.rowidx, , drop=FALSE]
		} else if(s == 4) {
			sheet = paste('Specific miRNA in ',Group[2],sep='')
			sheetData = miRNA.normalizedData.Comp[Group2.miRNA.rowidx, , drop=FALSE]
		} else if(s == 5) {
			sheet = paste('Common miRNA in ',Group[1],' & ',Group[2],sep='')
			sheetData = miRNA.normalizedData.Comp.stat
			sheetData = sheetData[order(sheetData[,'pvalue']), , drop=FALSE]
		} else if(s == 6) {
			sheet = paste('pvalue',' < ',pvalue.cutoff,sep='')
			sheetData = miRNA.normalizedData.Comp.stat[which(miRNA.normalizedData.Comp.stat[,'pvalue'] < pvalue.cutoff), , drop=FALSE]
			sheetData = sheetData[order(sheetData[,'pvalue']), , drop=FALSE]
		} else if(s == 7) {
			sheet = paste('pvalue',' >= ',pvalue.cutoff,sep='')
			sheetData = miRNA.normalizedData.Comp.stat[which(miRNA.normalizedData.Comp.stat[,'pvalue'] >= pvalue.cutoff), , drop=FALSE]
			...
			sheetData = miRNA.normalizedData.Comp.UPinGroup1
		} else if(s == 9) {
			sheet = paste(Group[1],' < ',Group[2],sep='')
			sheetData = miRNA.normalizedData.Comp.UPinGroup2
		} else if(s == 10) {
			sheet = 'Not available miRNA'
			sheetData = miRNA.normalizedData.Comp[NA.miRNA.rowidx, , drop=FALSE]
		}
			...
			if(nrow(sheetData) > 0)
			{
				...
				if(length(grep("sample information", sheet)) == 0) {
					if(length(grep("common", sheet, ignore.case=TRUE)) > 0 | length(grep("pvalue", sheet, ignore.case=TRUE)) > 0 | length(grep(">", sheet, ignore.case=TRUE)) > 0 | length(grep("<", sheet, ignore.case=TRUE)) > 0) {
						setColWidths(wb, sheet, cols=c(1:ncol(sheetData)), widths=c(10,rep(17.5,5),rep(12.5,ncol(sheetData)-6)))
						freezePane(wb, sheet, firstActiveRow=2, firstActiveCol=7+4)
					} else {
						setColWidths(wb, sheet, cols=c(1:ncol(sheetData)), widths=c(10,rep(17.5,5),rep(12.5,ncol(sheetData)-6)))
	...
	if(length(wb$sheet_names) > 0) {
		openxlsx::saveWorkbook(wb, file=paste('./Data/02_significant/miRNA/',Group[1],' vs ',Group[2],'/','miRNA_datatable ','(',Group[1],' vs ',Group[2],')','.xlsx',sep=''), overwrite=TRUE)
	}
	
	# --- write xlsx file by openxlsx package ---------------------------------------------------------
	if(m == 1) {
		wb2 <- openxlsx::createWorkbook("./Data/02_significant/miRNA/miRNA_datatable (pairwise comparison).xlsx")
	...
	for(s in start.idx:2)
	{
		...
		if(!is.null(sheetData))
		{
			if(nrow(sheetData) > 0)
			{
				...
				if(length(grep("sample information", sheet)) == 0) {
					if(length(grep("common", sheet, ignore.case=TRUE)) > 0 | length(grep("pvalue", sheet, ignore.case=TRUE)) > 0 | length(grep(">", sheet, ignore.case=TRUE)) > 0 | length(grep("<", sheet, ignore.case=TRUE)) > 0) {
						setColWidths(wb2, sheet, cols=c(1:ncol(sheetData)), widths=c(10,rep(17.5,5),rep(12.5,ncol(sheetData)-6)))
						freezePane(wb2, sheet, firstActiveRow=2, firstActiveCol=7+4)
					} else {
						setColWidths(wb2, sheet, cols=c(1:ncol(sheetData)), widths=c(10,rep(17.5,5),rep(12.5,ncol(sheetData)-6)))
						freezePane(wb2, sheet, firstActiveRow=2, firstActiveCol=7)
					}
				} else if(length(grep("sample information", sheet)) > 0) {
	...
	if(m == ncol(combn.idx)) {
		if(length(wb2$sheet_names) > 0) {
			openxlsx::saveWorkbook(wb2, file="./Data/02_significant/miRNA/miRNA_datatable (pairwise comparison).xlsx", overwrite=TRUE)
		}
	}
}
